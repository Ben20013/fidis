...
export default {
    ...
    created() {
        ...
        window.changeBreadcrum = function(data) {
            self.$emit("sendSubRoute", data);
        };
    },
    watch: {
        $route(to, from) {
    ...
    methods: {
        changeIframeUrl(subRoute) {
            this.iframeUrl = protocol + host + this.route + "#" + subRoute;
            console.log("iframe.vue------->iframeUrl: " + this.iframeUrl);
            document
                .getElementById("show-iframe")
                .contentWindow.location.reload(true);
        },
        changeBread(data) {
            this.$emit("sendSubRoute", data);
        }
    }
};
</script>
