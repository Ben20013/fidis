...
class MenuController extends AdminController{
    ...
    public function edit(Request $request){
        $info=$request->all();
        try{
            $info['display_system']='["'.implode('","', explode(',', $info['display_system'])).'"]';
    ...
    public function menuOption(Request $request){
        try {
            ...
            // 数据查询
                $data = $query->forPage($this->page_index, $this->page_size)
                    ->orderBy('listorder', 'asc')
                    ->get(['parentid', 'name as label', 'uid as value', 'uid', 'route'])->toArray();
                $data=$this->listToTree($data);
            }
    ...
    public function getMenuList(){
        try {
            ...
            if($query){
            // 数据查询
                $data = $query->forPage($this->page_index, 100)
                    ->orderBy('id', 'asc')
                    ->get(['*'])->toArray();
                $data=$this->listToTree($data);
                foreach($data as $key=>$val){
