...
use Modules\Admin\Services\Common\ModuleService;
use Modules\Admin\Services\Common\MenuService;
use Exception;
use Error;
use ZipArchive;
...
class ModuleController extends AdminController{
    ...
    public function __construct(ModuleService $ModuleService){
        ...
        $this->module_storage_path=MODULE_PATH.'..'.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'storage'.DIRECTORY_SEPARATOR.'modules';
    }
    ...
    public function generate(Request $request){
        $param=$request->all();
        try {
            $this->zipDir($this->module_path.DIRECTORY_SEPARATOR.$param['name'], $this->module_storage_path.DIRECTORY_SEPARATOR.$param['name'].'.fop');
            $result=array(
                'name'=>$param['name'],
                'url'=>''
            );
            $result = $this->module_service->formatMixResult(self::QUERY_CUSTOMER_SUCCESS, $result);
            return $this->module_service->formatReturnResult(200, trans('生成 模块安装包 成功'), $result);
        } catch(Exception  $exception) {
            $mix_code = self::QUERY_CUSTOMER_FAILED;
            $result = $this->module_service->formatMixResult($mix_code, [], $exception);
            return $this->module_service->formatReturnResult(500, trans('查询 模块信息 失败'), $result);
    ...
    public function test(){
        $this->read_dir($this->module_path.DIRECTORY_SEPARATOR.'Report');
    }
    //循环读取develop文件夹下文件
	private function read_dir($dir){
		...
		while(($file=readdir($handle))!=false){
			if($file!=".."&&$file!="."&&$file!='.gitkeep'&&$file!='node_modules'){
                print_r($dir.DIRECTORY_SEPARATOR.$file.'<br>');
				if(is_dir($dir.DIRECTORY_SEPARATOR.$file)){
					$files[]=array(
						'filetype'=>'folder',
						'text'=>$file,
						'path'=>$dir.DIRECTORY_SEPARATOR.$file.DIRECTORY_SEPARATOR,
						'children'=>$this->read_dir($dir.DIRECTORY_SEPARATOR.$file),
					);
				}else{
                    //print_r($file.'<br>');
					$files[] = array(
						...
						'url'=>'?m=admin&c=redevelop&a=init&menuid=&editor=true&filename='.$dir.DIRECTORY_SEPARATOR.$file,
					);
				}
			}
		}
