...
<template>
    <div class="status-content-wrap flex-column">
        ...
        <div class="page-btn flex-wrap">
            <div
                class="mix-btn"
                v-for="item in dashBoardPageLabel"
                :key="item.key"
                :class="currDashBoard==item.key?'active':''"
...
export default {
    ...
    mounted() {
        this.getEquipmentStatusConfig(this.mainEquipmentId);
    },
    methods: {
        getEquipmentStatusConfig(id) {
            ...
            deviceApi.get_equipment_status_conf(
                function(data) {
                    if (data.code == 200) {
                        ...
                        mixEcharts.drawDashBoard(
                            self.mainEquipmentId,
                            result,
        ...
        changeDashBoard(type) {
            ...
            if (mixEcharts.jsonSocket != null && !this.mainEquipmentId) {
                mixEcharts.dashBoardUnSubscribe(this.mainEquipmentId);
            }
            if (this.dashBoardConf != null) {
                mixEcharts.drawDashBoard(
                    this.mainEquipmentId,
                    this.dashBoardConf,
                    document.getElementById("dashBoardContent"),
                    this.currDashBoard,
                    false
    ...
    watch: {
        mainEquipmentId: ["getEquipmentStatusConfig"]
    },
    beforeDestroy() {
