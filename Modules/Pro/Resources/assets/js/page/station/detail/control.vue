      ...
      <div class="md-modal md-effect-1 control-passwd-win" :class="{ 'md-show': md.controlPwd}">
        <div class="md-content">
          ...
          <div class="titleBox flex-wrap flex-center">
            <div class="title">{{$t('stationLang.control_password_tip')}}</div>
            <div class="iconWrap" @click="colseControlPwdWin"><i class="close"></i></div>
          </div>
          <!--内容-->
          <div class="contBox">
            <div class="input-control-passwd">
              <el-input type="password" v-model="controlPassword" :placeholder="$t('stationLang.control_password_tip')"></el-input>
            </div>
          </div>
          <!-- 按钮 -->
		...
		<div class="md-modal-container">
			<div class="md-modal md-effect-1 modify-pwd-win"  :class="{ 'md-show': md.modifypwd}">
				<div class="md-content">
					<!--头部-->
					<div class="titleBox flex-wrap flex-center">
						<div class="title">{{$t('stationLang.modify_pwd')}}</div>
						<div class="iconWrap" @click="closeMd('modifypwd')"><i class="close"></i></div>
					</div>
					<div class="contentBox">
            <div class="flex-column">
              <div class="info-row">
                <div class="name">{{$t('stationLang.old_pwd')}}</div>
                <div class="value">
                  <el-input type="password" v-model="oldpassword" class="mix-input" auto-complete="new-password" :class="{'mix-active-input':oldpassword!=''}" :placeholder="$t('stationLang.please_input_old_pwd')"></el-input>
                </div>
              </div>
              <div class="info-row">
                <div class="name">{{$t('stationLang.new_pwd')}}</div>
                <div class="value">
                  <el-input type="password" v-model="newpassword" class="mix-input" auto-complete="new-password" :class="{'mix-active-input':newpassword!=''}" :placeholder="$t('stationLang.please_input_new_pwd')"></el-input>
                </div>
              </div>
              <div class="info-row">
                <div class="name">{{$t('stationLang.confirm_new_pwd')}}</div>
                <div class="value">
                  <el-input type="password" v-model="confirmnewpassword" class="mix-input" auto-complete="new-password" :class="{'mix-active-input':confirmnewpassword!=''}" :placeholder="$t('stationLang.please_input_new_pwd_once_more')"></el-input>
                </div>
              </div>
            </div>
					</div>
...
export default {
  ...
  mounted(){
    ...
    // mixControlCharts.drawDashBoard(this.mainEquipmentId, mixControlData, document.getElementById('controlDashboardContainer'))
  },
  methods:{
    getControlDashBoard() {
      deviceApi.get_dashboard(data => {
        if (data.code == 200) {
          ...
          if (typeof control == 'object' && !Array.isArray(control)) {
            mixControlCharts.drawDashBoard(this.mainEquipmentId, control, document.getElementById('controlDashboardContainer'))
          }
        } else {
          this.$message.error(data.msg)
        }
      }, {'equipment_id': this.mainEquipmentId, 'type': 'control'})
    },
    colseControlPwdWin(){
    ...
    checkControlPwd(){
      ...
      },{'equipment_id':this.mainEquipmentId,'auth_code':this.controlPassword})
    },
    sendControlCommand(){
      ...
      params.command.equipment_id = this.mainEquipmentId;
      ...
      deviceApi.push_control_command(function(data){
        if (data.code == 200) {
          self.$alert(self.$t('stationLang.control_command_success'),self.$t('stationLang.equipment_control'),{
            confirmButtonText:self.$t('el.messagebox.confirm'),
          })
          self.controlSwitchDom = null
        } else {
          let msg = data.msg ? data.msg : data.mix_msg
          self.$alert(data.msg,self.$t('stationLang.equipment_control'),{
            confirmButtonText:self.$t('el.messagebox.confirm'),
          })
    ...
    submitModifyPwd(){
      if (this.oldpassword == '') {
        this.toast(this.$t('stationLang.old_pwd_not_null'),'error');
        return;
      }
      if (this.newpassword == '') {
        this.toast(this.$t('stationLang.new_pwd_not_null'),'error');
        return;
      }
      if (this.newpassword != this.confirmnewpassword) {
        this.toast(this.$t('stationLang.twice_pwd_not_same'),'error');
        return;
      }
      let self = this;
      deviceApi.reset_control_auth(function(data){
        if (data.code == 200) {
          ...
          self.toast(self.$t('stationLang.modify_pwd_success'),'success');
        } else {
          self.toast(data.msg,'error');
        }
      },{'equipment_id':this.mainEquipmentId,'old_code':this.oldpassword,'new_code':this.newpassword})
    },
    addLog(operation){
      ...
      },{'type':6,'operation':operation,'primary_key':this.mainEquipmentId});
    },
    toast(msg,type){
  ...
  beforeDestroy(){
    if (mixControlCharts.jsonSocket!=null) {
      mixControlCharts.dashBoardUnSubscribe(this.mainEquipmentId);
      mixControlCharts.socketConnectCount = 10;
      mixControlCharts.isDestroyed = true;
      mixControlCharts.jsonSocket=null;
      console.log('dashBoardUnSubscribe:'+this.mainEquipmentId);
    }
  },
}
</script>
...
@import "../../../../sass/station/detail/control.scss";
</style>
