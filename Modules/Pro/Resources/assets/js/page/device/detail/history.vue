    ...
    <div class="content flex-column">
      <div class="button-tool flex-wrap">
        <div class="button-tool__left">
          <div class="tool__content flex-wrap" v-show="!isExportView">
            ...
            <div class="select-header-btn" @click="openSettingHeaderWin">
              <span>{{$t('deviceLang.select_params')}}</span>
            </div>
          ...
          <div class="tool__content flex-wrap" v-show="isExportView">
            ...
            <div class="tool__interval flex-wrap" :class="{'active': toolIntervalFocus}">
              <div class="tool__interval_txt">{{$t('deviceLang.interval')}}</div>
        ...
        <div class="button-tool__right flex-wrap">
          <div class="tool__tab-btn left" :class="{'active': !isExportView}" @click="handelChangeView(false)">{{$t('deviceLang.historical_data')}}</div>
          <div class="tool__tab-btn right" :class="{'active': isExportView}" @click="handelChangeView(true)">{{$t('deviceLang.export_data')}}</div>
        </div>
      </div>
      ...
      <div class="record-list" v-if="isExportView">
        <div class="table-wrap">
          ...
          height="100%">
            <el-table-column
              prop="task_start"
              :label="$t('deviceLang.task_start')"
              width="170">
            </el-table-column>
            <el-table-column
              prop="task_id"
              :label="$t('deviceLang.task_id')"
              width="290">
            </el-table-column>
            <el-table-column
              prop="filename"
              :label="$t('deviceLang.filename')"
              width="*">
            </el-table-column>
            <el-table-column
              prop="task_status"
              :label="$t('deviceLang.task_status')"
              width="150">
            </el-table-column>
            <el-table-column
              :label="$t('deviceLang.action')"
              width="120">
      ...
      <div class="md-modal md-effect-1 setting-table-header" :class="{ 'md-show': md.header}">
        <div class="md-content">
          ...
          <div class="titleBox flex-wrap flex-center">
            <div class="title">{{$t('deviceLang.select_params')}}</div>
            ...
            <div class="params__content flex-wrap">
              <div class="params__content_left flex-column">
                <div class="params__header flex-wrap">
                  <div class="header__item">{{$t('deviceLang.parameter_item')}}</div>
                  <div class="header__item flex-wrap">
                    <div class="header__item select-btn" @click="settingTableHeaderSelectAll">{{isSelectAllTitle?$t('deviceLang.select_all'):$t('deviceLang.cancel_select_all')}}</div>
              ...
              <div class="params__content_right flex-column">
                <div class="program__header flex-wrap">
                  <div class="header__item">{{$t('deviceLang.plan_item')}}</div>
                  <div class="header__item flex-wrap">
                    <div class="params__search">
                  ...
                  <div class="program__item flex-wrap" :class="{'active': selectedPlanIndex === index}" v-for="(item, index) in filterPlanData" :key="index">
                    <div class="item__name flex-wrap">
                      ...
                      <div class="item__name_txt">
                        ...
                        <input v-else class="item__name_input" type="text" v-model="item.name" :placeholder="$t('deviceLang.plan_name_tip')">
                      </div>
                    </div>
          ...
          <div class="buttomBox flex-wrap flex-center">
            <!-- <input type="button"  class="btn_cancel" :value="isSelectAllTitle?$t('deviceLang.select_all'):$t('deviceLang.cancel_select_all')"  @click="settingTableHeaderSelectAll" style="width:90px;"/> -->
    ...
    <div class="md-modal-container">
      <div class="md-modal md-effect-1 dataCurvewin" :class="{ 'md-show': md.curve}">
        <div class="md-content">
          <!--头部-->
          <div class="titleBox flex-wrap flex-center">
            <div class="title">{{$t('deviceLang.data_line')}}</div>
            ...
            <div class="data-curve-wrap">
              <div class="top-content flex-wrap">
                <el-radio-group v-model="datacurverradio" @change="dataCurverRadioChange">
                  <el-radio :label="0">{{$t('deviceLang.day')}}</el-radio>
                  <el-radio :label="1">{{$t('deviceLang.week')}}</el-radio>
                  <el-radio :label="2">{{$t('deviceLang.month')}}</el-radio>
                </el-radio-group>
                <div class="collect-time flex-wrap">
                  <span>{{$t('deviceLang.period')}}</span>
                  <div class="block">
                    <el-date-picker
                  ...
                  <!-- <div class="warn-btn" @click="showDataCurverAlarmPoint">
                    <span>{{ isShowAlarmPoint ?$t('deviceLang.hide_alarm_point'):$t('deviceLang.show_alarm_point') }}</span>
                  </div> -->
...
export default {
  ...
  methods:{
    ...
    getStateList() {
      ...
      }, {'equipment_id':this.equipmentId,'page_size':this.exportRecordCurrentPageSize,'page_index':this.exportRecordCurrentPage})
    },
    exportStateData() {
      if (this.exportDateTimeRange.length < 2) {
        this.$message.error(this.$t('deviceLang.select_time_period'))
        return
      }
      if (this.recordListSelectTitle.length == 0) {
        this.$message.error(this.$t('deviceLang.parameter_cannot_be_empty'))
        return;
      }
      ...
      }, {'equipment_id': this.equipmentId,'start_time': this.exportDateTimeRange[0],'end_time': this.exportDateTimeRange[1], 'resample': this.exportTimeInterval,'keys': JSON.stringify(this.recordListSelectTitle)})
    },
    ...
    deleteStateFile(task) {
      ...
      this.$confirm(this.$t('deviceLang.continue'), this.$t('deviceLang.tip'), {
        ...
        this.$message({
          type: 'info',
          message: this.$t('deviceLang.cancel_delete')
        });
      });
    },
    ...
    addPlanItem() {
      let item = {
        name: this.$t('deviceLang.new_plan') + (this.planData.length +1),
        isEdit: false,
        data: []
      }
    ...
    savePlanItem(item) {
      if (item.name.length > 10) {
        this.$message({
          message: this.$t('deviceLang.plan_name_tip'),
          type: 'warning'
        });
        return
      }
    ...
    addPlanItemSave(item) {
      deviceApi.add_scheme(data => {
        ...
        item.isEdit = false
      }, {'equipment_id': this.equipmentId, 'name': item.name, 'setting': JSON.stringify(item.data)})
    },
    editPlanItemSave(item) {
      deviceApi.edit_scheme(data => {
        ...
        item.isEdit = false
      }, {'equipment_id': this.equipmentId, 'uid': item.uid, 'name': item.name, 'setting': JSON.stringify(item.data)})
    },
    getPlanList() {
    ...
    openSettingHeaderWin(){
      if (!this.equipmentId) {
        this.$alert(this.$t('deviceLang.no_query_equipment'),this.$t('deviceLang.history_record'),{
          confirmButtonText:this.$t('el.messagebox.confirm'),
        })
        return;
      }
    ...
    getRecordListData(){
      if (this.recordListAllTitle.length==0 || !this.equipmentId) {
        this.$message.error(this.$t('deviceLang.parameter_error'))
        return;
      }
      if (this.recordListSelectTitle.length==0) {
        this.$message.error(this.$t('deviceLang.parameter_cannot_be_empty'))
      ...
      deviceApi.get_record_list_data(function(data){
        if(data.code=='200'){
          ...
          self.recordListTitle.unshift({'prop':'time','label':self.$t('deviceLang.collect_time')});
          // 拼装表格数据格式
          let result = data.result.data;
      ...
      },{'equipment_id':this.equipmentId,'start_time':this.historyRecordStartDateValue,'end_time':this.historyRecordEndDateValue,'page_size':15,'page_index':1,'keys':JSON.stringify(this.recordListSelectTitle)})
    },
    ...
    searchHistoryList(){
      if (!this.recordListSelectTitle || this.recordListSelectTitle.length==0||!this.equipmentId) {
        return false;
      }
      ...
      },{'equipment_id':this.equipmentId,'start_time':start_time,'end_time':end_time,'page_size':this.historyRecordCurrentPageSize,'page_index':this.historyRecordCurrentPage,'keys':JSON.stringify(this.recordListSelectTitle)})
    },
    refreshHistoryList(){
      this.setHistoryRecordTime();
      this.historyRecordCurrentPageSize = 15;
    ...
    importHistoryList(){
      ...
      if (ins>604800000) { //大于7天
        this.$alert(this.$t('deviceLang.import_history_tip'),this.$t('deviceLang.history_record'),{
          confirmButtonText:this.$t('el.messagebox.confirm'),
        })
        return;
      }
      ...
      deviceApi.import_record_list_data(function(data){
        ...
        // equipmentId
      },{'equipment_id':this.equipmentId,'start_time':this.historyRecordStartDateValue,'end_time':this.historyRecordEndDateValue,'page_size':999999999,'page_index':1,'keys':JSON.stringify(this.recordListSelectTitle),'title':JSON.stringify(title)})
    },
    openCurveWin(){
      this.getTableHeaderList();
      ...
      if (this.isSettingTableHeader) {
        ...
        },{'equipment_id':this.equipmentId,'start_time':this.datacurverTimeStart,'end_time':this.datacurverTimeEnd,'keys':JSON.stringify(val)})
      }
    },
    downDataCurverImg(){
      let echartsInstance = this.$echarts.getInstanceByDom(document.getElementById('dataCurveEcharts'));
    ...
    showDataCurverAlarmPoint(){
      ...
      },{'equipment_id':this.equipmentId,'start_time':this.datacurverTimeStart,'end_time':this.datacurverTimeEnd})
    },
    datacurverSearch(params){
      ...
      if (val.length == 0) {
        this.$alert(this.$t('deviceLang.data_line_tip'),this.$t('deviceLang.data_line'),{
          confirmButtonText:this.$t('el.messagebox.confirm'),
        });
        return;
      }
      ...
      },{'equipment_id':this.equipmentId,'start_time':start_time,end_time,'keys':JSON.stringify(val)})
    },
    datacurverSelectChange(type){
      ...
      if (obj.isExist) {
        this.$alert(this.$t('deviceLang.data_line_exist'),this.$t('deviceLang.data_line'),{
          confirmButtonText:this.$t('el.messagebox.confirm'),
        });
        return;
      }
...
<style rel="stylesheet/scss" lang="scss" scoped>
  @import '../../../../sass/device/detail/history.scss';
</style>
