        ...
        <div class="right-content-box flex-column">
            ...
            <div class="search-wrap">
                <div class="title-wrap flex-wrap flex-center">
                    ...
                    <div class="right-box flex-wrap ">
                        ...
                        <!-- <div class="mix-icon-txt-button" @click="handleClickAdd"><i class="mix-add-gray-icon"></i><span>体检任务</span></div> -->
                    </div>
                </div>
            </div>
            ...
            <div class="body-wrap flex-column">
                <div class="table-wrap flex-con-1 flex-column">
                    ...
                    height="100%" class="mix-el-table">
                        <el-table-column prop="task_id" label="任务编号"></el-table-column>
                        ...
                        <el-table-column label="操作" width="140">
                            <template slot-scope="scope">
                                <div class="table-operate">
                                    <div class="flex-wrap">
                                        <!-- <span class="mix-create-result-icon" @click="handleTableRowClickCreateResultByHand(scope.row)" title="手动体检"></span>
                                        <span class="mix-delete-icon" @click="handleTableRowClickDelete(scope.row)" title="删除"></span> -->
                                        <!-- <div class="mix-icon-button" @click.stop="handleTableRowClickDelete(scope.row)" title="删除"><i class="mix-delete-icon"></i></div> -->
                                        <div class="mix-icon-button" @click.stop="handleTableRowClickCreateResultByHand(scope.row)" title="手动体检"><i class="mix-create-result-icon"></i></div>
                ...
                <div class="pagination-box">
                    <my-pagination :total="searchObj.total_records" :pageSize="searchObj.page_size"
                        @pageChange="handlePageChange" @sizeChange="handleSizeChange"></my-pagination>
                </div>
            </div>
        </div>
...
<script>
    import diagTaskApi from "@/assets/js/fetch/diag/task.js"
    ...
    export default {
        ...
        methods: {
            ...
            initSearchCondition() {
                ...
                this.searchObj.search_key = 'task_name';
            ...
            handleClickAdd() {
                this.$router.push({name: 'equipmentTypeAdd'});
            },
            ...
            handleTableRowClickCreateResultByHand(row) {
                let self = this;
                this.openScreenLoading();
                diagTaskApi.createResultByHand( (res) => {
                    if (res.code == 200) {
                        self.$message({type: 'success', message: res.msg});
                        self.closeScreenLoading(400);
                        return ;
                    }
                    self.closeScreenLoading(400);
            ...
            handleTableRowClickDelete(row) {
                ...
                self.$confirm("确定删除id为" + row.task_id +"的体检任务？", "提醒", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                }).then( () => {
                    diagTaskApi.delete( (res) => {
                        if (res.code == 200) {
                            self.$message({type: 'success', message: res.msg});
                            self.toSearch();
                            return ;
                    ...
                    self.$message({type: 'info', message: "删除已取消"});
                });
...
.mix-create-result-icon {
    display: inline-block;
    width: 16px;
    height: 16px;
    // border-radius: 2px;
    // border: solid 1px #dddddd;
    background: url('../../../../../images/public/export-hand-icon.png') no-repeat center center;
}
